...
## Panneau Style/Palette/Preset masque en mode EN DIRECT — il ne pilotait rien

**Hors de docs/00a.** Aaron, apres avoir verifie que le mode fichier marchait
bien (redessin confirme sur changement de style ET de palette, testes
separement, y compris en pause pour isoler l'effet de l'animation), a demande
un test dans Beat Studio lui-meme. Ecart trouve immediatement : le panneau
Style/Palette/Preset de droite reste affiche pendant le direct, mais **cliquer
dessus ne change rigoureusement rien** au visuel affiche.

Cause, confirmee a l'execution avant tout correctif (`getComputedStyle`,
lecture de `window.__pulsarLiveDebug.sceneId` avant/apres clic) : `#live-canvas`
(mode direct, pilote par `LiveDirector`) est un calque `position:absolute`
empile PAR-DESSUS `#canvas` (mode fichier, celui que le panneau Style/Palette
controle reellement). Les deux existent en meme temps dans le DOM ; seul le
second ecoute ce panneau. Cliquer « Field » pendant le direct : `sceneId`
inchange, verifie directement.

**Correctif** : dans `src/ui/App.ts`, le listener `pulsar:live-offer` masque
desormais `<aside>` (le panneau complet, racine unique — confirme par
`grep '<aside' index.html`) des que `onTrack` demarre effectivement le rendu
live, et le rend a nouveau des que `onConnectionStateChange` signale
`closed`/`failed`/`disconnected`. Deux lignes, un `style.setProperty('display',
'none')` / `removeProperty('display')`, aucune classe CSS ajoutee — le panneau
n'a pas de `.hidden` generique, seulement des regles par id (`#dropzone.hidden`,
`#analysis-status.hidden`), donc une propriete inline directe plutot qu'une
classe reutilisee a tort.

Verifie en conditions reelles (Playwright + `file://` contre Beat Studio, comme
d'habitude) : `getComputedStyle(document.querySelector('aside')).display` vaut
`none` une fois la connexion WebRTC etablie et le rendu live demarre — capture
d'ecran a l'appui, le panneau a completement disparu, le canvas prend toute la
largeur.

`npx tsc --noEmit` : 0 erreur. `npx vitest run` : 1199/1199 verts (125/125
fichiers), aucune regression. `npm run build` : succes, 246 modules,
`index-Dh75m_Wy.js` 536,68 ko (gzip 154,07 ko).
Limites connues : le reaffichage sur deconnexion n'a pas ete verifie
explicitement en conditions reelles (la fermeture normale de l'overlay retire
l'iframe entierement, ce qui rend la question sans objet dans ce cas precis ;
le chemin `onConnectionStateChange` compte pour une coupure reseau SANS
fermeture de l'overlay, plus rare, non reproduite ici). Les boutons d'en-tete
(« Charger une demo », « Choisir un fichier », etc.) restent visibles et
cliquables en direct — hors perimetre de ce correctif, qui ne visait que le
